...
const TOOL_PROMPTS: Record<string, (input: string) => string> = {
  ...
  thesaurus: (input) =>
    `You are a poet's thesaurus. For the word "${input}", return exactly 10 synonyms. At the top give a short defintion of the word so the user knows its meaning. Vary them across tone and register — include formal, lyrical, archaic, and modern options. Format each entry like this:
    "${input}: [definition of word]
